...
struct SummaryReviewHost: View {
    ...
    var body: some View {
        ...
        let shown = example(from: Store().load().history)
        SummaryScreen(
            record: shown.record,
            celebration: shown.celebration,
    ...
    /// logic produces.
    /// A finished session to render, and the history it belongs to.
    ///
    /// The history is part of the answer rather than an input, because every
    /// tier below synthesises its own and a screen handed one without the other
    /// describes a session that never happened.
    ...
    private func example(from history: [SessionRecord]) -> Example {
        /// The celebration is always derived from the history it is shown with.
        func shown(_ record: SessionRecord, _ history: [SessionRecord]) -> Example {
            Example(
                record: record,
                celebration: Celebrations.forSession(record, history: history),
                history: history
            )
        }
        ...
        case "stalled":
            ...
            let sessions = (0 ..< 3).map { index -> SessionRecord in
                let log = [
