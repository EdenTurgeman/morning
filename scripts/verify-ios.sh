...
run "export format"    bash -c 'f="ios/build/export-check.json"; if [[ -f "$f" ]]; then node scripts/verify-export.mjs "$f"; else echo "the test run wrote no export to check" >&2; exit 1; fi'
run "workout doc"      python3 scripts/verify-workout-doc.py
