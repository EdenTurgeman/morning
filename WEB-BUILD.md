# The retired web build

**This is the README the web app shipped with, kept verbatim.** It describes
`src/` — the TypeScript/React PWA that ran this workout until the native iOS app
took over in September 2026, and which is still deployed at
<https://edenturgeman.github.io/morning/>.

**Everything in it that points at `src/program.ts` as the place to change the
workout is now wrong.** The program lives in `ios/Morning/Program.swift`, and
...
somebody takes it down. `plans/014-the-ios-app-takes-the-repo.md` sequences the
retirement.
